...
from utils.datasets import build_dataset,build_few_shot_dataset
from utils.pos_embed import interpolate_pos_embed
from utils.misc import NativeScalerWithGradNormCount as NativeScaler
from huggingface_hub import hf_hub_download, login
from engine_finetune_few_shot import train_one_epoch, evaluate
...
os.environ['TORCHELASTIC_WORK_DIR'] = '/data/wyy/Project/RETFound_MAE/torch_distribute_logs'
...
def get_args_parser():
    ...
    #labeling efficiency实验，用多少百分比的训练集，靠seed固定不同模型的训练集。
    parser.add_argument('--few_shot', type=float, default=1.0, help='percentage or number of samples of each class for training')
...
def main(args, criterion):
    ...
    if args.finetune and not args.eval:
        ...
        if args.model!='RETFound_mae' and args.model!='swin_large_patch16'  and args.model!='multi_scale_vit' :
    ...
    dataset_train,dataset_test = build_few_shot_dataset(args=args)
    args.classes = dataset_test.classes
    print(dataset_test.classes)
    print('train dataset shape: {}, {}'.format(dataset_train[0][0].shape, len(dataset_train)))
    print('test dataset shape: {} , {}'.format(dataset_test[0][0].shape, len(dataset_test)))
    ...
    if not args.eval:
        args.model_store_dir = os.path.join(args.output_dir, args.task, args.dataset_name + '_' + args.task + '_' + str(int(args.few_shot)) + '_' + str(args.seed))
    else:
        args.model_store_dir = os.path.dirname(args.resume)

    if global_rank == 0 and args.model_store_dir is not None and not args.eval:
        os.makedirs(args.model_store_dir, exist_ok=True)
        log_writer = SummaryWriter(log_dir=os.path.join(args.model_store_dir, 'logs'))
    else:
        log_writer = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats, test_score, test_roc, test_aupr, test_acc = evaluate(data_loader_test, model, device, args, epoch, mode='test',
                                        num_class=args.nb_classes, log_writer=log_writer)
        ...
        if log_writer is not None:
            log_writer.add_scalar('loss/test', test_stats['loss'], epoch)
        ...
        if args.output_dir and misc.is_main_process():
            if log_writer is not None:
                log_writer.flush()
            with open(os.path.join(args.model_store_dir, "log.txt"), mode="a", encoding="utf-8") as f:
